...
// Trả về mốc đầu tháng hiện tại và đầu tháng kế tiếp (dùng để lọc doanh thu
// "trong tháng" — vd hôm nay 02/07/2026 thì từ 01/07/2026 đến trước 01/08/2026).
// Vì đây là tính theo thời gian thực (query trực tiếp created_at trong bảng
// purchases), số liệu sẽ TỰ ĐỘNG chuyển sang tháng mới và về lại 0 khi sang
// tháng, không cần thao tác thủ công gì cả.
function getCurrentMonthRange() {
    const now = new Date();
    const start = new Date(now.getFullYear(), now.getMonth(), 1);
    const end = new Date(now.getFullYear(), now.getMonth() + 1, 1);
    return { start: start.toISOString(), end: end.toISOString(), label: `Tháng ${now.getMonth() + 1}/${now.getFullYear()}` };
}

async function adminFetchMonthlyRevenue() {
    const { start, end } = getCurrentMonthRange();
    const { data, error } = await sb.from('purchases')
        .select('amount')
        .gte('created_at', start)
        .lt('created_at', end);
    if (error) { console.error('adminFetchMonthlyRevenue:', error); return 0; }
    return (data || []).reduce((sum, p) => sum + Number(p.amount || 0), 0);
}

async function adminFetchStats() {
    const [{ count: userCount }, { count: pendingCount }, monthlyRevenue] = await Promise.all([
        sb.from('profiles').select('*', { count: 'exact', head: true }),
        sb.from('card_queue').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
        adminFetchMonthlyRevenue()
    ]);
    return {
        userCount: userCount || 0,
        pendingCount: pendingCount || 0,
        totalRevenue: monthlyRevenue // doanh thu của THÁNG HIỆN TẠI (không phải all-time nữa)
    };
}

// ---------------------------------------------------------------------
// ĐƠN HÀNG (ADMIN) — lưu trữ/tra cứu trong phạm vi 2 THÁNG GẦN NHẤT,
// có thể tìm theo email / số điện thoại / ID Discord của người mua.
// Lưu ý: dữ liệu gốc trong bảng "purchases" KHÔNG bị xoá, hàm này chỉ
// giới hạn phạm vi truy vấn hiển thị cho Admin trong 2 tháng gần nhất.
// ---------------------------------------------------------------------
async function adminSearchOrders(query, monthsBack = 2, limit = 200) {
    const since = new Date();
    since.setMonth(since.getMonth() - monthsBack);

    let req = sb.from('purchases')
        .select('*, profiles!inner(email, phone, discord_id)')
    ...
    const q = (query || '').trim();
    if (q) {
        req = req.or(`email.ilike.%${q}%,phone.ilike.%${q}%,discord_id.ilike.%${q}%`, { foreignTable: 'profiles' });
    }

    const { data, error } = await req;
    if (error) { console.error('adminSearchOrders:', error); return []; }
    return data || [];
}
